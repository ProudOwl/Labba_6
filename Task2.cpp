...
void generateRandomKey(vector<Byte>& key, size_t length = 16) {
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<> dis(0, 255); // Один байт - от 0 до 255
...
// генерация случайного вектора-блока инициализации (IV)
...
vector<Block> expandKey(const vector<Byte>& key) {
    ...
    while (bytesGenerated < 176) {
        ...
        if (bytesGenerated % 16 == 0) {
            ...
            for (int i = 0; i < 4; ++i)
                temp[i] = sbox[temp[i]];
            // Первый байт XORится с текущей константой Rcon.
    ...
    // Делим весь массив из expandedKey на отдельные блоки по 16 байт
...
// AddRoundKey. Реализована с помощью xorBlocks
